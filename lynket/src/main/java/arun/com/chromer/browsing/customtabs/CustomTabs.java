...
public class CustomTabs {
    ...
    switch (Preferences.get(activity).preferredAction()) {
      case PREFERRED_ACTION_BROWSER:
        ...
        if (Utils.isPackageInstalled(activity, pakage)) {
          ...
          final PendingIntent openBrowserPending;
          if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            openBrowserPending = PendingIntent.getBroadcast(activity, 0, intent, PendingIntent.FLAG_MUTABLE);
          }else{
            openBrowserPending = PendingIntent.getBroadcast(activity, 0, intent, FLAG_UPDATE_CURRENT);
          }
      ...
      case PREFERRED_ACTION_FAV_SHARE:
        ...
        if (Utils.isPackageInstalled(activity, pakage)) {
          ...
          final PendingIntent favSharePending;
          if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            favSharePending = PendingIntent.getBroadcast(activity, 0, intent, PendingIntent.FLAG_MUTABLE);
          }else{
            favSharePending = PendingIntent.getBroadcast(activity, 0, intent, FLAG_UPDATE_CURRENT);
          }
      ...
      case PREFERRED_ACTION_GEN_SHARE:
        ...
        final PendingIntent sharePending;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
          sharePending = PendingIntent.getBroadcast(activity, 0, intent, PendingIntent.FLAG_MUTABLE);
        }else{
          sharePending = PendingIntent.getBroadcast(activity, 0, intent, FLAG_UPDATE_CURRENT);
        }
  ...
  private void prepareChromerOptions() {
    ...
    PendingIntent moreMenuPending =  null ;

    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
      moreMenuPending = PendingIntent.getActivity(activity, 0, moreMenuActivity, PendingIntent.FLAG_MUTABLE);
    }else{
      moreMenuPending = PendingIntent.getActivity(activity, 0, moreMenuActivity, FLAG_UPDATE_CURRENT);
    }
  ...
  private void prepareMinimize() {
    if (!Preferences.get(activity).bottomBar() && Utils.ANDROID_LOLLIPOP) {
      ...
      final PendingIntent pendingMin;
      if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
        pendingMin = PendingIntent.getBroadcast(activity, new Random().nextInt(), minimizeIntent, PendingIntent.FLAG_MUTABLE);
      }else{
        pendingMin = PendingIntent.getBroadcast(activity, new Random().nextInt(), minimizeIntent, PendingIntent.FLAG_UPDATE_CURRENT);
      }
    ...
    switch (Preferences.get(activity).preferredAction()) {
      case PREFERRED_ACTION_BROWSER:
        ...
        if (Utils.isPackageInstalled(activity, pkg)) {
          ...
          if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            pendingShareIntent = PendingIntent.getBroadcast(activity, 0, shareIntent, PendingIntent.FLAG_MUTABLE);
          }else{
            pendingShareIntent = PendingIntent.getBroadcast(activity, 0, shareIntent, FLAG_UPDATE_CURRENT);
          }
      ...
      case PREFERRED_ACTION_FAV_SHARE:
        ...
        if (Utils.isPackageInstalled(activity, pkg)) {
          if (!pkg.equalsIgnoreCase(STABLE_PACKAGE)) {
            ...
            final PendingIntent pendingBrowseIntent;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
              pendingBrowseIntent = PendingIntent.getBroadcast(activity, 0, browseIntent, PendingIntent.FLAG_MUTABLE);
            }else{
              pendingBrowseIntent = PendingIntent.getBroadcast(activity, 0, browseIntent, FLAG_UPDATE_CURRENT);
            }
  ...
  private void prepareCopyLink() {
    ...
    final PendingIntent serviceIntentPending;
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
      serviceIntentPending = PendingIntent.getBroadcast(activity, 0, clipboardIntent, PendingIntent.FLAG_MUTABLE);
    }else {
      serviceIntentPending = PendingIntent.getBroadcast(activity, 0, clipboardIntent, FLAG_UPDATE_CURRENT);
    }
  ...
  private void prepareOpenInChrome() {
    ...
    if (Utils.isPackageInstalled(activity, customTabPkg)) {
      if (customTabPkg.equalsIgnoreCase(BETA_PACKAGE)
        ...
        final PendingIntent openChromePending;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
          openChromePending = PendingIntent.getBroadcast(activity, 0, chromeReceiver, PendingIntent.FLAG_MUTABLE);
        }else {
          openChromePending = PendingIntent.getBroadcast(activity, 0, chromeReceiver, FLAG_UPDATE_CURRENT);
        }
  ...
  private void prepareOpenWith() {
    ...
    final PendingIntent openWithActivityPending;
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
      openWithActivityPending = PendingIntent.getActivity(activity, 0, openWithActivity, PendingIntent.FLAG_MUTABLE);
    }else {
      openWithActivityPending = PendingIntent.getActivity(activity, 0, openWithActivity, FLAG_UPDATE_CURRENT);
    }
